...
export class AdminTableComponent {
  ...
  resetPassword(id: number): void {
    this.userService.resetUserPassword(id).subscribe(
      (response: any) => {
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'User password is reset successfully!.' });
  ...
  getFilteredUsers(users: User[]): void {
    users.some((user) => user.roles.some((role) => {
      if (this.checkRole("ROLE_SUPER_ADMIN") ||
        (this.checkRole("ROLE_ICMS_ADMIN") && ((role.name.indexOf('ROLE_ICMS_BRANCH') !== -1 || role.name.indexOf('ROLE_ICMS_PROVISION') !== -1 || role.name.indexOf('ROLE_ICMS_BRANCH_MANAGER') !== -1 || role.name.indexOf('ROLE_ICMS_DISTRICT') !== -1 || role.name.indexOf('ROLE_ICMS_ADMIN') !== -1))) ||
